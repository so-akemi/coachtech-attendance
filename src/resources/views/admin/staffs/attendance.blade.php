...
@section('content')
    <div class="attendance-content">
        ...
        <div class="attendance-month-nav">
            <a href="{{ route('admin.attendance.staff', ['id' => $user->id, 'month' => $prevMonth]) }}" class="month-nav-link">← 前月</a>
            <span class="month-nav-current">
                <span class="calendar-icon material-symbols-outlined">calendar_month</span>
                {{ $currentMonth->format('Y/m') }}
            </span>
            <a href="{{ route('admin.attendance.staff', ['id' => $user->id, 'month' => $nextMonth]) }}" class="month-nav-link">翌月 →</a>
        </div>
